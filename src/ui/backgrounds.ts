...
/**
 * What sits behind the screens: a set of patterns the game draws for itself,
 * or a photo of the player's own. Ported from killer-sudoku, which took them
 * from Sudoku Variants, so the same six patterns are on offer in every
 * DanDoku game, drawn from the same paths.
 *
 * The patterns are SVG built at load and handed to CSS as data URIs — nothing
 * is downloaded, so the game stays offline from the first run, and they are a
 * few hundred bytes each. A photo is shrunk to fit on a canvas, re-encoded as
 * a JPEG and kept in localStorage on the device it was chosen on; it never
 * leaves.
 */
...
/** Under Kakuro's own prefix: the DanDoku games share one origin. */
const PHOTO_KEY = 'kk:v1:background';
