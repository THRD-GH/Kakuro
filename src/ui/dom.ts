 ...
 * and days answer that — nobody is deciding anything on the difference between
 * 37 and 41 minutes.
 */
...
/**
 * Which build this is, and when it was made, in the reader's own time zone.
 * Worth showing because a PWA can keep serving an older build until the
...
/**
 * A press that has to be held, for the buttons that throw work away. Fires on
 * a long press or a double click, and never on an ordinary tap — the same
 * guard the rest of the collection puts on Clear.
 */
export function onHold(node: HTMLElement, hold: () => void, tap?: () => void): void {
